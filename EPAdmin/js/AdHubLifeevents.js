...
$(document).ready(function() {
	...
	GetPastEvents()
	 ...
	 $("#cal").datepicker({
            ...
            buttonImage: 'https://infornt.sharepoint.com/sites/RNTENG/SiteAssets/ENG-Admin/images/icons8-calendar-24.png',
            changeMonth: true,
            changeYear: true,
            dateFormat: 'mm/yy',
            onSelect: function (dateString, txtDate) {
                console.log("Selected Date: " + dateString + "\nTextBox ID: " + txtDate.id);
                var startOfMonth = moment(dateString).format("MM-YYYY");
				var endOfMonth = moment(dateString).endOf('month').toISOString();
...
function GetUpcomingEvents(today) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=Pillar eq 'Life' and EventDate ge '"+today+"'&$orderby=Created desc",
       method: "GET",
       headers:
          {
