 ...
 $(document).ready(function() { 
   ...
   $("#SeriessaveNow").click(function() {SaveSeriesSaveNow()});
 ...
 function SaveSeriesSaveNow() { 
   ...
   createNewItemWithAttachmentsSeriesSaveNow("EPEvent", data).then( 
		function() { 
 ...
 var createNewItemWithAttachmentsSeriesSaveNow = function(listName, listValues) { 
	 ...
	 context.executeQueryAsync(function() { 
		 ...
		 if (listValues[0].Files.length != 0) { 
			 if (fileCountCheck <= listValues[0].Files.length - 1) { 
				 loopFileUploadSeriesSaveNow(listName, id, listValues, fileCountCheck).then(function() {}, 
		 			 function(sender, args) { 
 ...
 function loopFileUploadSeriesSaveNow(listName, id, listValues, fileCountCheck) { 
 var dfd = $.Deferred(); 
 SeriesuploadFileHolderSeriesSaveNow(listName, id, listValues[0].Files[fileCountCheck].Attachments).then( 
 function (data) { 
 ...
 fileCountCheck++; 
 if (fileCountCheck <= listValues[0].Files.length - 1) { 
 loopFileUploadSeriesSaveNow(listName, id, listValues, fileCountCheck); 
 } else { 
...
// if (oLoader.close) setTimeout(function () { oLoader.close();  }, 3000); 
 ...
 function SeriesuploadFileHolderSeriesSaveNow(listName, id, file) { 
	 //file = $(this)[0].files[0];
 ...
 getFileBufferSeriesSaveNow(file).then(function(buffer) {
 ...
 function getFileBufferSeriesSaveNow(file) { 
 var deferred = $.Deferred(); 
 var reader = new FileReader(); 
 reader.onload = function(e) { 
 deferred.resolve(e.target.result); 
