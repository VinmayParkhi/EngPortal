...
function GetWorkBanner() {
    ...
    ({
        ...
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPBanner')/items?$top=1&select=AttachmentFiles,Title,BannerDescription,Pillar&$expand=AttachmentFiles&$filter=Pillar eq 'Work'&$orderby=Created desc",
        method: "GET",
           ...
           //console.log(pillar);
            //if(pillar == "Work"){
                ...
                //$(".workBannerTop").append('<img src="'+ a +'" width="100" style="border-radius: 50%;"/>');
 				$(".workBannerTop").attr('style','border-radius:1.5rem;background:linear-gradient(-90deg, #00000000 0%, #121112 21%, #000000 90%),url('+ a +');background-position:100%; background-repeat: no-repeat; background-size: contain !important;margin-bottom: 1rem;');
