...
function GetPastEvents() {
 	var today= moment().format('YYYY-MM-DDTHH:mm:SS');    
 	$.ajax({
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Enrich') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"') and (VideoURL ne null)&$orderby=EventDate desc",
        method: "GET",
        headers:
           {
               "Accept": "application/json;odata=verbose"
