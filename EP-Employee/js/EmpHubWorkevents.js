    ...
    var userId = _spPageContextInfo.userId;
       $.ajax({
        ...
        success: function (data, status, xhr) {
            ...
            if(results.length > 0){
              //$("#heartImg").attr("src","images/Group 3212.svg");
...
function GetPastEvents() {
 	var today= moment().format('YYYY-MM-DDTHH:mm:SS');    
 	$.ajax({
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Work') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"')and (VideoURL ne null)&$orderby=EventDate desc",
        method: "GET",
        headers:
           {
               "Accept": "application/json;odata=verbose"
               ...
               console.log("video is",VideoURLs);
				...
				if(AverageRating  == null){
 				...
 				$(".pastEvntsWr1").append('<div class="col-12 col-md-6 col-lg-4 card pastevntsCard"><div class="pstvideoImg1" style="height: auto;background-image:linear-gradient(180deg, #00000000 0%, #0D0D0DE8 74%, #121112 100%), url('+b+');background-repeat: no-repeat;background-position: center top;background-size: 100% 100%;display: flex; flex-direction: column;justify-content: center;align-items: center;">'+ 
							...
							'src="images/Group 3218.svg" alt="Heart"'+ 'data-themekey="#" style="width: 60%;"></a> </div> <div class="timeDiv"><label class="time">15:03</label> </div> </div>'+
...
function FavEvents(EventTitle,pillar,EventDesc,imageURL,id,event){
 		...
 		var ex = event.target;
 		//ex.setAttribute("src", "images/Group 3212.svg")
 		log(username)
 		var userId = _spPageContextInfo.userId;
...
EvID = id;
    ...
    ({
       ...
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventStartTime,EventEndTime,EventDescription,EventDate,VideoURL&$expand=AttachmentFiles&$filter= Pillar eq 'Work' and ID eq '"+id+"'",
        method: "GET",
        headers:
           {
               // Accept header: Specifies the format for response data from the server.
