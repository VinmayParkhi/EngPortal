...
var EventTime;
 ...
 var getBell = document.querySelector('.bell');
  ...
  $("#setReminderBtnData").on("click", function() { 
         setReminderDataHub();
...
function setReminderDataHub(){
		EventDt2 = $("div.upcomingeventsCalendar a label").html();
		reminderTitle = $("#EventTitle").html();
        ...
        var eventStartDate = moment(selectTimeValueNew).format("YYYY-MM-DDTHH:mm:SS");
		log(eventStartDate)
        var SetURL = "https://outlook.office.com/calendar/0/deeplink/compose?subject="+reminderMessage+"&body="+reminderTitle+"&startdt="+eventStartDate+"&enddt="+EventDt2+"&location=India&path=%2Fcalendar%2Faction%2Fcompose&rru=addevent"
 ...
 clientContext.executeQueryAsync(Function.createDelegate(this, this.onQuerySucceeded), Function.createDelegate(this, this.onQueryFailed))
  ...
  $.ajax({
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,AttachmentFiles,Attachments,Pillar,EventLocation,Title,EventDescription,EventLink,EventDate,EventStartTime,EventEndTime&$expand=AttachmentFiles",
        method: "GET",
        headers:
           {
        ...
        if(results[i].ID === id){
         ...
         var Location = results[i].EventLocation;
	   ...
	   GetEventImg.setAttribute('src',"https://amdocs.sharepoint.com/" + EventBannerImg)
