...
=======
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,AttachmentFiles,EventLink,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,EventEndTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Wow and pride') and (Status eq 'Published') and (EventEndTime ge '"+today+"')&$orderby=EventDate asc",
>>>>>>> bdf0b129be5c2250ab0e559a940604f01c6b56b8
        method: "GET",
        headers:
           {
...
function GetPastEvents() {
 	var today= moment().format('YYYY-MM-DDTHH:mm:SS');    
 	$.ajax({
       // url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,EventEndTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Wow and pride') and (EventEndTime le '"+today+"')&$orderby=Created desc",
<<<<<<< HEAD
               url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,AttachmentFiles,EventLink,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,EventEndTime,VideoURL&$expand=AttachmentFiles&$&$filter=(Pillar eq 'Wow and pride') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"')&$orderby=EventDate desc",
=======
               url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,AttachmentFiles,EventLink,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,EventEndTime,VideoURL&$expand=AttachmentFiles&$&$filter=(Pillar eq 'Wow and pride') and (Status eq 'Completed') and (EventEndTime le '"+today+"')&$orderby=EventDate desc",
>>>>>>> bdf0b129be5c2250ab0e559a940604f01c6b56b8
        ...
        success: function (data, status, xhr) {
            ...
            for (var i = 0; i < 3; i++) {
               EventDates = moment(data.d.results[i].EventDate).format('YYYY-MM-DD');  
              // EventDatesPast = moment(data.d.results[i].EventEndTime).format('YYYY-MM-DD hh:mm a');              
               ...
               console.log("video is",VideoURLs);
				...
				if(AverageRating  == null){
 				...
 				$(".pastEvntsWr1").append('<div class="col-12 col-md-6 col-lg-4 card pastevntsCard"><div class="pstvideoImg1" style="height: auto;background-image:linear-gradient(180deg, #00000000 0%, #0D0D0DE8 74%, #121112 100%), url('+b+');background-repeat: no-repeat;background-position: center top;background-size: 100% 100%;display: flex; flex-direction: column;justify-content: center;align-items: center;">'+ 
                            ...
                            '<a id="1" lblId ="popuprate-h5" class="reviewPopup" onclick="getEPlistData(this.id,event)" href="#" class="reviewPopup" lblId = "popuprate-h5" data-bs-toggle="modal" data-bs-target="#ratereviewModal">Write Review</a></h6>'+
