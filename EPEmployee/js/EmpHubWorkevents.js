...
function addZero(i) {
 ...
 function getTitleName(EvTitle) {
  ...
  $('#minutes').change(function(){
      var remindBefore = document.getElementById('minutes').value;
      var reminderStartTime = startTime;
      var reminderDate = new Date(reminderStartTime);
       
       console.log(reminderDate.getHours()+':'+reminderDate.getMinutes());
...
function GetUpcomingEvents() {
    ...
    $.ajax({
<<<<<<< HEAD
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,AttachmentFiles,EventLink,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,EventEndTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Work') and (EventStatus eq 'Published') and (EventEndTime ge '"+today+"')&$orderby=EventDate asc",
=======
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,AttachmentFiles,EventLink,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,EventEndTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Work') and (Status eq 'Published') and (EventEndTime ge '"+today+"')&$orderby=EventDate asc",
>>>>>>> bdf0b129be5c2250ab0e559a940604f01c6b56b8
        method: "GET",
        headers:
           {
               ...
               VideoURLs = data.d.results[i].VideoURL;
				...
				$("#popupEVDesc").html(EventDescription);
                 ...
                 $(".upcomingEventsWr").append('<div class="col-12 col-md-6 col-lg-4 card upcomingEventsCard"> <div class="upcomingeventsImg"  style=" display: flex; flex-direction: column; justify-content: center; align-items: start; "> <div class="userImg1"'+
												...
												' </div><div class="upcomingeventsBtn" style="display: flex; align-items: center; justify-content: space-between; width: 100%; height: auto;padding: 0 0.8rem"> <div class="upcomingeventsReminder"><a href="#" onclick="getTitleName(this.name);" class="setReminder" data-bs-toggle="modal" data-bs-target="#HubWbModal" id="HubWbReminder" name="'+EventTitle+'">Set Reminder</a></div> <div class="upcomingeventsJoin"> <a href="'+EventLink+'" style="color:#fff;">Join</a></div></div> </div></div></div>');
...
function GetPastEvents() {
 	...
 	$.ajax({
<<<<<<< HEAD
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Work') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"')&$orderby=EventDate desc",
=======
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Work') and (Status eq 'Completed') and (EventEndTime le '"+today+"')&$orderby=EventDate desc",
>>>>>>> bdf0b129be5c2250ab0e559a940604f01c6b56b8
        method: "GET",
        headers:
           {
        ...
        success: function (data, status, xhr) {
            ...
            for (var i = 0; i < 3; i++) {
              // EventDates = moment(data.d.results[i].EventDate).format('YYYY-MM-DD');               
               ...
               var EventTime = data.d.results[i].EventStartTime;
               var EvStartTime = moment.utc(data.d.results[i].EventStartTime).format('hh:mm a');
               var EventId = data.d.results[i].ID;
               pillar = data.d.results[i].Pillar;
