...
=======
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,AttachmentFiles,EventLink,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,EventEndTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Connect and community') and (Status eq 'Published') and (EventEndTime ge '"+today+"')&$orderby=EventDate asc",
...
function GetPastEvents() {
 	...
 	$.ajax({
<<<<<<< HEAD
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Connect and community') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"')&$orderby=EventDate desc",
=======
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=(Pillar eq 'Connect and community') and (Status eq 'Completed') and (EventEndTime le '"+today+"')&$orderby=EventDate desc",
>>>>>>> bdf0b129be5c2250ab0e559a940604f01c6b56b8
        method: "GET",
        headers:
           {
               ...
               console.log("video is",VideoURLs);
				...
				if(AverageRating  == null){
 				...
 				$(".pastEvntsWr1").append('<div class="col-12 col-md-6 col-lg-4 card pastevntsCard"><div class="pstvideoImg1" style="height: auto;background-image:linear-gradient(180deg, #00000000 0%, #0D0D0DE8 74%, #121112 100%), url('+b+');background-repeat: no-repeat;background-position: center top;background-size: 100% 100%;display: flex; flex-direction: column;justify-content: center;align-items: center;">'+ 
							...
							'<a href="#" onclick="FavEvents(this.id,this.className,this.title,this.name,this.hreflang,event);" hreflang="'+EventId+'" title="'+EventDescription+'" id="'+pillar+'" name="'+imageURL+'" class="'+EventTitle+'" style="display: flex; justify-content: center;"> <img id="heartImg"'+ 
