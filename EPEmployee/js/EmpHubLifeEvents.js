...
function GetUpcomingEvents() {
    ...
    $.ajax({
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,AttachmentFiles,Attachments,Pillar,Title,EventDescription,AverageRating,EventDate,EventStartTime,EventEndTime,VideoURL&$expand=AttachmentFiles&$filter=Pillar eq 'Life' and EventDate ge '"+today+"'&$orderby=Created desc",
        method: "GET",
        headers:
           {
...
function GetPastEvents() {
 	...
 	today = moment(today).toISOString();
    $.ajax({
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,AverageRating,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=Pillar eq 'Life' and EventDate le '"+today+"'&$orderby=Created desc",
        method: "GET",
        headers:
           {
               "Accept": "application/json;odata=verbose"
        ...
        success: function (data, status, xhr) {
            ...
            for (var i = 0; i <3; i++) {
               ...
               console.log("video is",VideoURLs);
                ...
                $(".pastEvntsWr1").append('<div class="pastevntsCard" style="height: auto; "><div class="pstvideoImg1" style="height: 31vh;background-image:linear-gradient(180deg, #00000000 0%, #0D0D0DE8 74%, #121112 100%), url('+b+');background-repeat: no-repeat;background-size: cover;"> <label class="time">15:03</label>'+
                            ...
                            '<a href="#" onclick="FavEvents(this.id,pillar['+i+'],EventTitle['+i+'],EventDesc['+i+']);AddClass(this);" id="'+EventId+'" class="heartImg"> <img id="heartImg" src="https://infornt.sharepoint.com/SiteAssets/ENGEmployee/IMAGES/Group 3218.svg" alt="Heart"></a>'+  
