...
function GetUpcomingEvents(today) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Wow and Pride') and (EventStatus eq 'Published') and (EventEndTime ge '"+today+"')&$orderby=Created desc",
       method: "GET",
       headers:
          {
	              ...
	              VideoURLs = data.d.results[i].VideoURL;
	               ...
	               $("#popupEVDesc").html(EventDescription);
	                ...
	                $(".eventlist").append('<div id="upcomEvent" class="col-4 card uce"> <span class="img"> <img src="'+a+'" alt=""></span> <div class="card-data"> <small><label id="pillar" class="'+pillar+'">'+pillar+'</label></small><span>'+EventTitle+'</span>'+
	                                       ...
	                                       '<span class="card-left"> <a href="#"><img src="https://amdocs.sharepoint.com/sites/EP/SiteAssets/ENG-Admin/Images/exited.png" alt=""><small>'+ExcitedMoodData+'</small></a>'+
	                                       '<a href="#"><img src="https://amdocs.sharepoint.com/sites/EP/SiteAssets/ENG-Admin/images/boring.png" alt=""><small>'+BoringMoodData+'</small></a>'+
	                                       '<a href="#"><img src="https://amdocs.sharepoint.com/sites/EP/SiteAssets/ENG-Admin/images/frustrated.png" alt=""><small>'+FrustetedMoodData+'</small></a></span>'+
...
function CompletedEvents(sc,ec) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=Pillar eq 'Wow and Pride' and EventDate ge '"+sc+"' and EventDate le '"+ec+"'&$orderby=Created desc",
       method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
...
function SelectedDateUpcomingEvents(sd,ed) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=Pillar eq 'Wow and pride' and EventDate ge '"+sd+"' and EventDate le '"+ed+"'&$orderby=Created desc",
       method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
...
function GetPastEvents() {
    ...
    var today= moment().format('YYYY-MM-DDTHH:mm:SS');
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Wow and pride') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"') and (VideoURL ne null)&$orderby=Created desc",
        method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
