       ...
       success: function (data, status, xhr) {
           ...
           if(dataresults.length > 0 ){
	           for (var i = 0; i < 3; i++) {
	              ...
	              var ExcitedMoodData= data.d.results[i].Excited;
	              ...
	              VideoURLs = data.d.results[i].VideoURL;
	               ...
	               $("#popupEVDesc").html(EventDescription);
	                ...
	                $(".eventlist").append('<div id="upcomEvent" class="col-4 card uce"> <span class="img"> <img src="'+a+'" alt=""></span> <div class="card-data"> <small><label id="pillar" class="'+pillar+'">'+pillar+'</label></small><span>'+EventTitle+'</span>'+
	                                       '<p id="EventDesc">'+EventDescription+'</p> <span style="display:flex;"> <span class="date" style="display:flex; align-items:baseline;"><img style="width:10%; margin-right:6px;" src="https://amdocs.sharepoint.com/sites/EP/SiteAssets/EngEmployee/images/Icon feather-calendar.png" alt="Mail">'+
	                                       ...
	                                       '<span class="card-left"> <a href="#"><img src= "../../SiteAssets/ENG-Admin/Images/exited.png" alt=""><small>'+ExcitedMoodData+'</small></a>'+
...
function CompletedEvents(sc,ec) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=Pillar eq 'Work' and and EventDate ge '"+sc+"' and EventDate le '"+ec+"'&$orderby=Created desc",
       method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
...
function SelectedDateUpcomingEvents(sd,ed) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=Pillar eq 'Work' and EventDate ge '"+sd+"' and EventDate le '"+ed+"'&$orderby=Created desc",
       method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
              ...
              VideoURLs = data.d.results[i].VideoURL;
               ...
               $("#popupEVDesc").html(EventDescription);
                ...
                $(".eventlist").append('<div class="card me-3 my-2 monthlyEvent"> <span class="img"> <img src="'+a+'" alt=""></span> <div class="card-data"> <small><label id="pillar" class="'+pillar+'">'+pillar+'</label></small><span>'+EventTitle+'</span>'+
                                       ...
                                       '<label>'+EventFullDates+'</label> </span> <span class="timedata" style="display:flex; align-items:baseline;"><img style="width:14%; margin-right:6px;" src="https://amdocs.sharepoint.com/sites/EP/SiteAssets/ENG-Admin/Images/Icon feather-clock.svg" alt="Message">'+localDate+'</span> </span> <span class="card-bottom">'+
...
function GetCompleteEvents() {
    ...
    var today= moment().format('YYYY-MM-DDTHH:mm:SS');
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Work') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"')&$orderby=Created desc",
        method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
...
function GetPastEvents() {
    ...
    var today= moment().format('YYYY-MM-DDTHH:mm:SS');
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Work') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"') and (VideoURL ne null)&$orderby=Created desc",
        method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
               ...
               $("#popupEVDesc").html(EventDescription);
                ...
                $(".pastEventsWr").append('<div class="pasteventsCard de"> <div class="row align-items-center justify-content-center videoImg1" style="height: 23vh;background-image:linear-gradient(180deg, #00000000 0%, #0D0D0DE8 74%, #121112 100%), url('+b+');background-repeat: no-repeat;background-size: cover;">'+
                						  '<a class="col-12 play player" style="cursor:pointer;width: 24%;" onclick="getVid(event)">'+
                               			  '<img src="https://amdocs.sharepoint.com/sites/EP/SiteAssets/EngEmployee/images/Group 2834.svg" alt="Play" style="width: 100%;"> </a>'+
