...
function GetUpcomingEvents(today) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Connect and community') and (EventStatus eq 'Published') and (EventEndTime ge '"+today+"')&$orderby=Created desc",
       method: "GET",
       headers:
          {
...
function SelectedDateUpcomingEvents(sd,ed) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=Pillar eq 'Connect and community' and EventDate ge '"+sd+"' and EventDate le '"+ed+"'&$orderby=Created desc",
       method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
              ...
              VideoURLs = data.d.results[i].VideoURL;
               ...
               $("#popupEVDesc").html(EventDescription);
                ...
                $(".eventlist").append('<div class="card me-3 my-2 monthlyEvent"> <span class="img"> <img src="'+a+'" alt=""></span> <div class="card-data"> <small><label id="pillar" class="'+pillar+'">'+pillar+'</label></small><span>'+EventTitle+'</span>'+
                                       ...
                                       '<span class="card-left"> <a href="#"><img src="images/excited.png" alt=""><small>'+ExcitedMoodData+'</small></a>'+
                                       '<a href="#"><img src="images/boring.png" alt=""><small>'+BoringMoodData+'</small></a>'+
                                       '<a href="#"><img src="images/frustrated.png" alt=""><small>'+FrustetedMoodData+'</small></a></span>'+
...
function GetPastEvents() {
    ...
    var today= moment().format('YYYY-MM-DDTHH:mm:SS');
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Connect and community') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"') and (VideoURL ne null)&$orderby=Created desc",
        method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
