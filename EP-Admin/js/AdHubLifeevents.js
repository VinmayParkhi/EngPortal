...
function GetPastEvents() {
    ...
    var today= moment().format('YYYY-MM-DDTHH:mm:SS');
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Life') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"') and (VideoURL ne null)&$orderby=Created desc",
        method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
...
console.log("sankedfaf")
	...
	//var evetit = event.target.parentElement.nextElementSibling.firstElementChild.firstElementChild.innerText;
		var evetit = event.target.parentElement.nextElementSibling.firstElementChild.lastElementChild.innerText;
