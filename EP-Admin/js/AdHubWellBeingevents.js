...
function GetUpcomingEvents(today) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Enrich') and (EventStatus eq 'Published') and (EventEndTime ge '"+today+"')&$orderby=Created desc",
       method: "GET",
       headers:
          {
       ...
       success: function (data, status, xhr) {
           ...
           if(dataresults.length > 0 ){
	           for (var i = 0; i <3; i++) {
	              EventFullDates = moment.utc(data.d.results[i].EventDate).format('DD MMM, YYYY');
...
function CompletedEvents(sc,ec) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=ID,Attachments,Pillar,Title,EventDescription,EventDate,EventStartTime,VideoURL&$expand=AttachmentFiles&$filter=Pillar eq 'Work' and EventDate ge '"+sc+"' and EventDate le '"+ec+"'&$orderby=Created desc",
       method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
...
function SelectedDateUpcomingEvents(sd,ed) {
   ...
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=Pillar eq 'Enrich' and EventDate ge '"+sd+"' and EventDate le '"+ed+"'&$orderby=Created desc",
       method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
...
function GetCompleteEvents() {
    ...
    var today= moment().format('YYYY-MM-DDTHH:mm:SS');
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Enrich') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"')&$orderby=Created desc",
        method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
...
function GetPastEvents() {
    ...
    var today= moment().format('YYYY-MM-DDTHH:mm:SS');
   $.ajax({
       url: _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getByTitle('EPEvent')/items?$select=*,Attachments&$expand=AttachmentFiles&$filter=(Pillar eq 'Enrich') and (EventStatus eq 'Completed') and (EventEndTime le '"+today+"') and (VideoURL ne null)&$orderby=Created desc",
        method: "GET",
       headers:
          {
              "Accept": "application/json;odata=verbose"
       ...
       success: function (data, status, xhr) {
           ...
           for (var i = 0; i <3; i++) {
              ...
              VideoURLs = data.d.results[i].VideoURL;
              console.log("VideoURL : "+VideoURLs)
                   //document.getElementById('VideoURL').setAttribute("src","VideoURLs");
                  $('iframe').attr("src",VideoURLs);
               $("#popupEVTitle").html(EventTitle);				
               $("#popupEVDesc").html(EventDescription);
                ...
                $(".player").attr('data-target',"#exampleModal");
           			...
           			$(".pastEventsWr").append('<span>no results</span>');	
           }
       },
       error: function (xhr, status, error) {
           console.log("Failed");
